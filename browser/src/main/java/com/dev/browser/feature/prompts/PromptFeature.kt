...
import com.dev.browser.feature.prompts.ChoiceDialogFragment.Companion.MENU_CHOICE_DIALOG_TYPE
import com.dev.browser.feature.prompts.ChoiceDialogFragment.Companion.MULTIPLE_CHOICE_DIALOG_TYPE
import com.dev.browser.feature.prompts.ChoiceDialogFragment.Companion.SINGLE_CHOICE_DIALOG_TYPE
...
class PromptFeature(
    ...
    fun onActivityResult(requestCode: Int, resultCode: Int, intent: Intent?) {
        if (requestCode == FILE_PICKER_ACTIVITY_REQUEST_CODE) {
            sessionManager.selectedSession?.promptRequest?.consume {
                ...
                val request = it as PromptRequest.File
    ...
    internal fun onPermissionsDenied() {
        sessionManager.selectedSession?.apply {
            promptRequest.consume { request ->
                if (request is PromptRequest.File) {
                    request.onDismiss()
                }
                true
            }
    ...
    internal fun onCancel(sessionId: String) {
        ...
        session.promptRequest.consume {
            when (it) {
                is PromptRequest.Alert -> it.onDismiss()
