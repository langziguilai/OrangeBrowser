    ...
    private void parseSecretCode(String code) {
        ...
        switch (param[0]) {
            case "show_nav_shelves":
                ReaderApplication.getConfigPreferences().edit().putBoolean("showNavShelves", enable).apply();
                msg = "已" + (enable ? "启" : "禁") + "用侧边栏书架！";
                RxBus.get().post(RxBusTag.RECREATE, true);
                break;
            case "fade_tts":
                ReaderApplication.getConfigPreferences().edit().putBoolean("fadeTTS", enable).apply();
                msg = "已" + (enable ? "启" : "禁") + "用朗读时淡入淡出！";
                break;
            case "use_regex_in_new_rule":
                ReaderApplication.getConfigPreferences().edit().putBoolean("useRegexInNewRule", enable).apply();
                msg = "已" + (enable ? "启" : "禁") + "用新建替换规则时默认使用正则表达式！";
                break;
            case "blur_sim_back":
                ReaderApplication.getConfigPreferences().edit().putBoolean("blurSimBack", enable).apply();
                msg = "已" + (enable ? "启" : "禁") + "用仿真翻页背景虚化！";
                break;
            case "async_draw":
                ReaderApplication.getConfigPreferences().edit().putBoolean("asyncDraw", enable).apply();
                msg = "已" + (enable ? "启" : "禁") + "用异步加载！";
                break;
            case "disable_scroll_click_turn":
                ReaderApplication.getConfigPreferences().edit().putBoolean("disableScrollClickTurn", enable).apply();
