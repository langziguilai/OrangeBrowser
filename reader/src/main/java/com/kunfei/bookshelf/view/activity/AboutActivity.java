...
public class AboutActivity extends MBaseActivity {
    ...
    TextView tvVersion;
    @BindView(R2.id.vw_version)
    CardView vwVersion;
    @BindView(R2.id.tv_donate)
    TextView tvDonate;
    @BindView(R2.id.vw_donate)
    CardView vwDonate;
    @BindView(R2.id.tv_scoring)
    TextView tvScoring;
    @BindView(R2.id.vw_scoring)
    CardView vwScoring;
    @BindView(R2.id.tv_git)
    TextView tvGit;
    @BindView(R2.id.vw_git)
    CardView vwGit;
    @BindView(R2.id.tv_disclaimer)
    TextView tvDisclaimer;
    @BindView(R2.id.vw_disclaimer)
    CardView vwDisclaimer;
    @BindView(R2.id.ll_content)
    LinearLayout llContent;
    @BindView(R2.id.tv_mail)
    TextView tvMail;
    @BindView(R2.id.vw_mail)
    CardView vwMail;
    @BindView(R2.id.tv_update)
    TextView tvUpdate;
    @BindView(R2.id.vw_update)
    CardView vwUpdate;
    @BindView(R2.id.tv_qq)
    TextView tvQq;
    @BindView(R2.id.vw_qq)
    CardView vwQq;
    @BindView(R2.id.tv_app_summary)
    TextView tvAppSummary;
    @BindView(R2.id.tv_update_log)
    TextView tvUpdateLog;
    @BindView(R2.id.vw_update_log)
    CardView vwUpdateLog;
    @BindView(R2.id.tv_home_page)
    TextView tvHomePage;
    @BindView(R2.id.vw_home_page)
    CardView vwHomePage;
    @BindView(R2.id.tv_faq)
    TextView tvFaq;
    @BindView(R2.id.vw_faq)
    CardView vwFaq;
    @BindView(R2.id.tv_share)
    TextView tvShare;
    @BindView(R2.id.vw_share)
    CardView vwShare;
    ...
    protected void bindView() {
        ...
        setupActionBar();
        tvVersion.setText(getString(R.string.version_name, ReaderApplication.getVersionName()));
    }
